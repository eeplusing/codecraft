...
public class Tsp
{
	private int cityNum ;    //城市个数
	 ...
	 private long[][] distance = new long[100][100];//城市的数量最好不要超过100个
	 ...
	 public Tsp() 
	 {
	  ...
	  String s = JOptionPane.showInputDialog("请输入城市的个数：");     //由于最后要回到原来的城市，输入城市时最好多算一个
	  cityNum = Integer.parseInt(s);
	...
	////////////////////////////////////////////// private ////////////////////////////////////////////////////////////
	 ...
	 private void initDistance(){
	  for(int i = 0; i < cityNum; i++ ) {       //为了输入的更快当 i = j 时自动的赋值为0      
	   distance[i][i] = 0;
	  }
	  for (int i = 0; i < cityNum ; i++) {           //由于二维数组的存储是对称的可以用压缩存储的方法初始化
	   for (int j = i + 1; j < cityNum; j++){
	     String s1 = JOptionPane.showInputDialog("请输入第"+ i +"城市到第"+ j +"城市之间的距离");
	     distance[i][j] = Integer.parseInt(s1);
